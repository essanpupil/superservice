...
def index():
    ...
    try:
        ...
        booksFromPolls = requests.get(POLL_URL + "/bookapi/books").json()
    except Exception as err:
        app.logger.error("Could not connect to Polls: %s", err)
        question = "Could not connect to Polls: {}".format(err)
        books = "Could not connect to BookAPI: {}".format(err)
        booksFromPolls = "Could not connect to BookAPI: {}".format(err)
